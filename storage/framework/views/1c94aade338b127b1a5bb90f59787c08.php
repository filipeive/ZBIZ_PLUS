<?php $__env->startSection('title', 'Concluir Pedido #' . $order->id); ?>
<?php $__env->startSection('page-title', 'Finalização da Encomenda #' . $order->id); ?>
    ...
    <div class="flex items-center justify-between bg-slate-900/80 border border-slate-800 rounded-3xl p-5 shadow-xl backdrop-blur-xl">
        <div>
            <h2 class="text-lg font-black font-heading text-white flex items-center gap-2">
                ...
                Concluir & Despachar Pedido #<?php echo e($order->id); ?>

            </h2>
            <p class="text-xs text-slate-400">Cliente: <?php echo e($order->customer_name); ?></p>
        </div>
        <a href="<?php echo e(route('orders.show', $order)); ?>" class="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold text-xs rounded-xl flex items-center gap-2 transition">
    ...
    <div class="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl backdrop-blur-xl space-y-6">
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 p-4 rounded-2xl bg-slate-950 border border-slate-800 text-xs">
            <div>
                ...
                <span class="text-sm font-bold text-white font-mono">MT <?php echo e(number_format($order->estimated_amount, 2, ',', '.')); ?></span>
            </div>
            <div>
                <span class="text-slate-500 block text-[10px] uppercase font-bold">Sinal Adiantado</span>
                <span class="text-sm font-bold text-emerald-400 font-mono">MT <?php echo e(number_format($order->advance_payment, 2, ',', '.')); ?></span>
            </div>
            <div>
                <span class="text-slate-500 block text-[10px] uppercase font-bold">Saldo a Liquidar</span>
                <span class="text-base font-black text-rose-400 font-mono">MT <?php echo e(number_format($remainingAmount, 2, ',', '.')); ?></span>
            </div>
        </div>

        <form action="<?php echo e(route('orders.process-completion', $order)); ?>" method="POST" class="space-y-4">
            <?php echo csrf_field(); ?>
            ...
            <div x-show="actionType === 'create_debt'" class="p-4 rounded-2xl bg-slate-950 border border-slate-800 space-y-3" style="display: none;">
                ...
                <input type="date" name="debt_due_date" class="w-full px-3.5 py-2.5 bg-slate-900 border border-slate-800 rounded-xl text-white text-xs" min="<?php echo e(date('Y-m-d')); ?>">
                <p class="text-[11px] text-amber-400 flex items-center gap-1">
                    <i class="fa-solid fa-triangle-exclamation"></i> Será registada uma nova dívida em nome de <?php echo e($order->customer_name); ?>.
                </p>
            </div>

            <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-800">
                <a href="<?php echo e(route('orders.show', $order)); ?>" class="px-4 py-2.5 bg-slate-800 text-slate-300 font-bold text-xs rounded-xl">Cancelar</a>
                <button type="submit" class="px-5 py-2.5 rounded-2xl <?php echo e($theme['btn']); ?> text-xs transition">Confirmar Conclusão</button>
            </div>
        </form>
    </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/fdev-ms/Filipe/ZBIZ_PLUS/resources/views/orders/complete.blade.php ENDPATH**/ ?>