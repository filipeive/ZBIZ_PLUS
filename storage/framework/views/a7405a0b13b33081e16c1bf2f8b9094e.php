<?php $__env->startSection('title', 'Relatório de Devedores'); ?>
<?php $__env->startSection('page-title', 'Relatório Consolidado de Devedores'); ?>
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-slate-900/80 border border-slate-800 rounded-3xl p-5 shadow-xl backdrop-blur-xl">
        ...
        <div class="flex items-center gap-3">
            ...
            <a href="<?php echo e(route('debts.report', array_merge(request()->query(), ['export' => 'excel']))); ?>" class="px-4 py-2 rounded-xl bg-emerald-500/20 text-emerald-400 border border-emerald-500/30 font-bold text-xs hover:bg-emerald-500/30 transition flex items-center gap-2">
    ...
    <div class="bg-slate-900/90 border border-slate-800 rounded-3xl p-5 shadow-xl backdrop-blur-xl">
        <form method="GET" action="<?php echo e(route('debts.report')); ?>" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4 items-end">
            <div>
                ...
                <select name="debt_type" onchange="this.form.submit()" class="w-full px-3.5 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-white text-xs">
                    ...
                    <option value="product" <?php echo e(request('debt_type') === 'product' ? 'selected' : ''); ?>>Produtos</option>
                    <option value="money" <?php echo e(request('debt_type') === 'money' ? 'selected' : ''); ?>>Dinheiro</option>
                </select>
            </div>
            <div>
            ...
            <div>
                ...
                <input type="text" name="customer" value="<?php echo e(request('customer')); ?>" placeholder="Nome do cliente/funcionário..." class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-white text-xs">
            </div>
            <div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div class="p-5 rounded-3xl bg-slate-900/90 border border-slate-800 shadow-lg backdrop-blur-xl">
            ...
            <div class="text-2xl font-black font-mono text-rose-400"><?php echo e(number_format($totalDebt, 2, ',', '.')); ?> MT</div>
        </div>
        <div class="p-5 rounded-3xl bg-slate-900/90 border border-slate-800 shadow-lg backdrop-blur-xl">
            <div class="text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1">Devedores Registados</div>
            <div class="text-2xl font-black font-mono text-white"><?php echo e($totalDebtors); ?></div>
            <div class="text-[10px] text-slate-500 mt-1"><?php echo e($productDebtors); ?> clientes • <?php echo e($moneyDebtors); ?> outros</div>
        </div>
        <div class="p-5 rounded-3xl bg-slate-900/90 border border-slate-800 shadow-lg backdrop-blur-xl">
            <div class="text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1">Média por Devedor</div>
            <div class="text-2xl font-black font-mono text-amber-400"><?php echo e(number_format($averageDebt, 2, ',', '.')); ?> MT</div>
        </div>
        <div class="p-5 rounded-3xl bg-slate-900/90 border border-slate-800 shadow-lg backdrop-blur-xl">
            <div class="text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1">Contas Vencidas</div>
            <div class="text-2xl font-black font-mono text-rose-500"><?php echo e($overdueDebtors); ?></div>
            <div class="text-[10px] text-slate-500 mt-1"><?php echo e(number_format($overdueDebtors / max($totalDebtors, 1) * 100, 1)); ?>% do total</div>
        </div>
    </div>
    ...
    <div class="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 shadow-xl backdrop-blur-xl overflow-hidden">
                ...
                <tbody class="divide-y divide-slate-800/60">
                    <?php $__empty_1 = true; $__currentLoopData = $debtors; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $debtor): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-slate-800/30 transition">
                            <td class="py-3.5">
                                <div class="font-bold text-white"><?php echo e($debtor->debtor_name); ?></div>
                                <?php if($debtor->debtor_phone): ?>
                                    <div class="text-[11px] text-slate-500"><?php echo e($debtor->debtor_phone); ?></div>
                                <?php endif; ?>
                            </td>
                            <td class="py-3.5">
                                <span class="px-2 py-0.5 rounded-full text-[10px] font-bold <?php echo e($debtor->debt_type === 'product' ? 'bg-blue-500/10 text-blue-400 border border-blue-500/30' : 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/30'); ?>">
                                    <?php echo e($debtor->debt_type === 'product' ? 'Produtos' : 'Dinheiro'); ?>

                                </span>
                            </td>
                            <td class="py-3.5 text-center font-bold text-slate-300"><?php echo e($debtor->debt_count); ?></td>
                            <td class="py-3.5 text-right font-mono font-black <?php echo e($debtor->status_group === 'Vencida' ? 'text-rose-400' : 'text-amber-400'); ?>">
                                <?php echo e(number_format($debtor->total_debt, 2, ',', '.')); ?> MT
                            </td>
                            <td class="py-3.5 text-center font-mono text-slate-400">
                                <?php echo e($debtor->oldest_debt ? \Carbon\Carbon::parse($debtor->oldest_debt)->format('d/m/Y') : '-'); ?>

                            </td>
                            <td class="py-3.5 text-center">
                                <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold <?php echo e($debtor->status_group === 'Vencida' ? 'bg-rose-500/10 text-rose-400 border border-rose-500/30' : 'bg-amber-500/10 text-amber-400 border border-amber-500/30'); ?>">
                                    <?php echo e($debtor->status_group); ?>

                                </span>
                            </td>
                            <td class="py-3.5 text-right">
                                <a href="<?php echo e(route('debts.index', ['customer' => $debtor->debtor_name])); ?>" class="inline-flex items-center px-3 py-1 bg-slate-800 hover:bg-slate-700 text-white rounded-xl text-xs font-bold transition">
                                    Ver Faturas
                                </a>
                            </td>
                        </tr>
        ...
        <?php if(method_exists($debtors, 'links')): ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/fdev-ms/Filipe/ZBIZ_PLUS/resources/views/debts/debtors-report.blade.php ENDPATH**/ ?>