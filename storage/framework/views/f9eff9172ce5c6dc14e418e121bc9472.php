    ...
    <header class="bg-slate-900 text-white px-4 py-2 flex items-center justify-between shadow-md">
        <div class="flex items-center space-x-3">
            ...
            <span class="bg-slate-800 text-xs px-2 py-1 rounded border border-slate-700 text-slate-300">
                <i class="fa-solid fa-store mr-1 text-emerald-400"></i><?php echo e(current_branch()?->name ?? 'Balcão Principal'); ?>

            </span>
            <span class="bg-slate-800 text-xs px-2 py-1 rounded border border-slate-700 text-slate-300">
                <i class="fa-solid fa-user mr-1 text-sky-400"></i><?php echo e(auth()->user()?->name ?? 'Operador'); ?>
        ...
        <div class="flex items-center space-x-4">
            ...
            <a href="<?php echo e(route('dashboard.index')); ?>" class="text-xs bg-slate-800 hover:bg-slate-700 text-slate-300 px-3 py-1.5 rounded transition">
        ...
        <div class="w-3/5 flex flex-col border-r border-gray-300 bg-white">
            ...
            <div class="flex overflow-x-auto p-2 bg-gray-100 gap-1 border-b border-gray-200 scrollbar-thin">
                ...
                <button @click="selectedCategory = <?php echo e($cat->id); ?>; searchProducts()"
                        :class="selectedCategory === <?php echo e($cat->id); ?> ? 'bg-slate-900 text-white' : 'bg-white text-gray-700 hover:bg-gray-200'"
    ...
    <div x-cloak x-show="showCustomerModal" class="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
        <div class="bg-white rounded-xl shadow-2xl w-full max-w-md p-6 space-y-4" @click.outside="showCustomerModal = false">
            ...
            <div class="space-y-2 max-h-60 overflow-y-auto">
                ...
                <button @click="customer = { id: <?php echo e($c->id); ?>, name: '<?php echo e($c->name); ?>', nuit: '<?php echo e($c->nuit); ?>' }; showCustomerModal = false"
                        class="w-full text-left p-2 rounded hover:bg-gray-100 text-xs border flex justify-between">
                    <span class="font-bold"><?php echo e($c->name); ?></span>
                    <span class="text-gray-400">NUIT: <?php echo e($c->nuit ?? 'N/D'); ?></span>
                </button>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </div>
        </div>
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('posApp', () => ({
                ...
                selectedType: 'all',
                tenantId: <?php echo e((int) ($tenantId ?? 0)); ?>,
                branchId: <?php echo e((int) ($branchId ?? 0)); ?>,
                storageScope: 'tenant-<?php echo e((int) ($tenantId ?? 0)); ?>-branch-<?php echo e((int) ($branchId ?? 0)); ?>',
