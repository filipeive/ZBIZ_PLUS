<?php $__env->startSection('title', 'Detalhes do Fiado #' . $debt->id); ?>
<?php $__env->startSection('page-title', 'Extrato da Dívida / Fiado #' . $debt->id); ?>
...
<div class="max-w-4xl mx-auto space-y-6" x-data="{ showPayModal: false }">
    ...
    <div class="flex items-center justify-between bg-slate-900/80 border border-slate-800 rounded-3xl p-5 shadow-xl backdrop-blur-xl">
        ...
        <?php if($debt->remaining_amount > 0): ?>
            <button @click="showPayModal = true" class="px-5 py-2 rounded-xl bg-gradient-to-r <?php echo e($theme['gradient']); ?> text-slate-950 font-black text-xs shadow-lg transition flex items-center gap-2">
    ...
    <div class="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl backdrop-blur-xl space-y-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-800">
            <div>
                <span class="px-3 py-1 rounded-full text-[10px] font-bold uppercase border <?php echo e($debt->remaining_amount <= 0 ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/30' : 'bg-amber-500/10 text-amber-400 border-amber-500/30'); ?> inline-flex items-center gap-1 mb-2">
                    <?php echo e($debt->remaining_amount <= 0 ? 'Liquidado na Totalidade' : 'Pendente de Pagamento'); ?>

                </span>
                <h2 class="text-2xl font-black font-heading text-white">Cliente: <?php echo e($debt->customer_name ?? $debt->customer?->name); ?></h2>
                <div class="text-xs text-slate-400 mt-1">Contacto: <?php echo e($debt->customer_phone ?? $debt->customer?->phone ?? 'N/D'); ?></div>
            </div>

            <div class="sm:text-right">
                <div class="text-xs uppercase font-bold text-slate-400">Saldo Devedor Atual</div>
                <div class="text-3xl font-black font-heading font-mono <?php echo e($debt->remaining_amount <= 0 ? 'text-emerald-400' : 'text-rose-400'); ?>">
                    <?php echo e(number_format($debt->remaining_amount, 2, ',', '.')); ?> <span class="text-xs text-slate-400">MT</span>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 p-4 rounded-2xl bg-slate-950 border border-slate-800 text-xs">
            <div>
                ...
                <div class="font-bold text-white font-mono text-sm mt-0.5"><?php echo e(number_format($debt->original_amount ?? $debt->total_amount, 2, ',', '.')); ?> MT</div>
            </div>
            <div>
                <div class="text-slate-500 font-bold uppercase text-[10px]">Total Já Amortizado</div>
                <div class="font-bold text-emerald-400 font-mono text-sm mt-0.5"><?php echo e(number_format($debt->paid_amount ?? 0, 2, ',', '.')); ?> MT</div>
            </div>
            <div>
                <div class="text-slate-500 font-bold uppercase text-[10px]">Data Limite / Vencimento</div>
                <div class="font-bold text-white mt-0.5"><?php echo e($debt->due_date ? \Carbon\Carbon::parse($debt->due_date)->format('d/m/Y') : 'Não estipulada'); ?></div>
            </div>
        </div>
                ...
                <table class="w-full text-left text-xs">
                    ...
                    <tbody class="divide-y divide-slate-800/60 bg-slate-900/60">
                        <?php $__empty_1 = true; $__currentLoopData = $debt->payments ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $payment): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                            <tr>
                                <td class="p-3 font-mono text-slate-400"><?php echo e($payment->created_at ? $payment->created_at->format('d/m/Y H:i') : '-'); ?></td>
                                <td class="p-3 uppercase text-slate-300"><?php echo e($payment->payment_method ?? 'Dinheiro'); ?></td>
                                <td class="p-3 text-slate-400"><?php echo e($payment->user?->name ?? 'Caixa'); ?></td>
                                <td class="p-3 text-right font-black text-emerald-400 font-mono"><?php echo e(number_format($payment->amount, 2, ',', '.')); ?> MT</td>
                            </tr>
    ...
    <div x-cloak x-show="showPayModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
        <div @click.away="showPayModal = false" class="bg-slate-900 border border-slate-800 rounded-3xl p-6 max-w-md w-full shadow-2xl space-y-4">
            ...
            <form action="<?php echo e(route('debts.payment', $debt->id)); ?>" method="POST" class="space-y-4">
                <?php echo csrf_field(); ?>
                <div>
                    <label class="block text-xs font-bold text-slate-300 mb-1">Montante a Pagar (MT) *</label>
                    <input type="number" step="0.01" max="<?php echo e($debt->remaining_amount); ?>" name="amount" value="<?php echo e($debt->remaining_amount); ?>" required
                ...
                <div class="flex gap-3 pt-2">
                    ...
                    <button type="submit" class="w-2/3 py-2.5 rounded-xl bg-gradient-to-r <?php echo e($theme['gradient']); ?> text-slate-950 font-black text-xs shadow-lg transition">Confirmar Pagamento</button>
                </div>
            </form>
        </div>
    </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/fdev-ms/Filipe/ZBIZ_PLUS/resources/views/debts/show.blade.php ENDPATH**/ ?>