<?php $__env->startSection('title', 'Categorias de Despesas'); ?>
<?php $__env->startSection('page-title', 'Categorias de Despesas & Classificação'); ?>
...
<div class="space-y-6" x-data="{ 
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-slate-900/80 border border-slate-800 rounded-3xl p-5 shadow-xl backdrop-blur-xl">
        ...
        <div class="flex items-center gap-3">
            <button @click="editMode = false; catId = null; catName = ''; catDesc = ''; catOperational = false; catRent = false; showModal = true" 
                    class="px-5 py-2.5 rounded-2xl <?php echo e($theme['btn']); ?> text-xs hover:scale-105 active:scale-95 transition flex items-center gap-2">
        ...
        <?php $__empty_1 = true; $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
            <div class="bg-slate-900/90 border border-slate-800 rounded-3xl p-5 shadow-lg backdrop-blur-xl flex flex-col justify-between hover:border-slate-700 transition group">
                <div>
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <div class="flex items-center gap-1">
                            <button @click="editMode = true; catId = <?php echo e($category->id); ?>; catName = '<?php echo e(addslashes($category->name)); ?>'; catDesc = '<?php echo e(addslashes($category->description ?? '')); ?>'; catOperational = <?php echo e($category->is_operational ? 'true' : 'false'); ?>; catRent = <?php echo e($category->is_rent ? 'true' : 'false'); ?>; showModal = true"
                            ...
                            <form action="<?php echo e(route('expense-categories.destroy', $category)); ?>" method="POST" onsubmit="return confirm('Tem certeza que deseja excluir esta categoria?')" class="inline">
                                <?php echo csrf_field(); ?>
                    ...
                    <h3 class="text-sm font-bold text-white mb-1"><?php echo e($category->name); ?></h3>
                    <p class="text-[11px] text-slate-400 line-clamp-2 mb-3"><?php echo e($category->description ?? 'Sem descrição adicional'); ?></p>

                    <div class="flex flex-wrap gap-1 mb-3">
                        <?php if($category->is_operational): ?>
                ...
                <div class="border-t border-slate-800/80 pt-3 flex items-center justify-between text-[11px] text-slate-500">
                    <span><?php echo e($category->expenses_count ?? $category->expenses()->count()); ?> despesa(s)</span>
                    <a href="<?php echo e(route('expense-categories.show', $category)); ?>" class="text-emerald-400 hover:underline font-bold">Ver Ficha</a>
                </div>
            </div>
    ...
    <div x-cloak x-show="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
        <div @click.away="showModal = false" class="bg-slate-900 border border-slate-800 rounded-3xl p-6 max-w-md w-full shadow-2xl space-y-4">
            ...
            <form :action="editMode ? `/expense-categories/${catId}` : '<?php echo e(route('expense-categories.store')); ?>'" method="POST" class="space-y-4">
                <?php echo csrf_field(); ?>
                ...
                <div class="flex gap-3 pt-3 border-t border-slate-800">
                    ...
                    <button type="submit" class="w-2/3 py-2.5 rounded-xl <?php echo e($theme['btn']); ?> text-xs transition" x-text="editMode ? 'Atualizar Categoria' : 'Guardar Categoria'"></button>
                </div>
            </form>
        </div>
    </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/fdev-ms/Filipe/ZBIZ_PLUS/resources/views/expenses/categories/index.blade.php ENDPATH**/ ?>