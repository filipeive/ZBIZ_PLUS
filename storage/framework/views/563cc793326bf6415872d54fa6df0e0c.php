<?php $__env->startSection('title', 'Certificado de Licença - ' . $tenant->name); ?>
<?php $__env->startSection('page-title', 'Certificado de Licença de Software'); ?>
...
<div class="max-w-4xl mx-auto space-y-6" x-data="{
    ...
    <div class="flex items-center justify-between gap-4 bg-slate-900/80 border border-slate-800 rounded-3xl p-5 shadow-xl backdrop-blur-xl print:hidden">
        <div class="flex items-center gap-3">
            <a href="<?php echo e(route('owner.tenants.show', $tenant)); ?>" class="px-4 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white text-xs font-bold transition flex items-center gap-2">
                <i class="fa-solid fa-arrow-left"></i> Voltar ao Tenant
            </a>
            <span class="text-xs text-slate-400">Emissão para <strong class="text-white"><?php echo e($tenant->name); ?></strong></span>
        </div>
    ...
    <div class="bg-gradient-to-b from-slate-900 via-slate-950 to-slate-900 border-2 border-emerald-500/40 rounded-3xl p-8 sm:p-12 shadow-2xl relative overflow-hidden print:border-slate-300 print:bg-white print:text-black print:p-6 print:shadow-none">
        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between border-b border-slate-800 print:border-slate-300 pb-8 gap-6">
            ...
            <div class="sm:text-right">
                ...
                <p class="text-[11px] text-slate-500 font-mono mt-1 print:text-slate-600">Ref: ZB-LIC-<?php echo e(str_pad($license->id, 6, '0', STR_PAD_LEFT)); ?></p>
            </div>
        </div>
        ...
        <div class="py-8 space-y-8">
            ...
            <div class="p-6 rounded-2xl bg-slate-950 border-2 border-dashed border-emerald-500/40 text-center space-y-2 print:bg-slate-50 print:border-slate-400">
                ...
                <div class="text-2xl sm:text-3xl font-black font-mono tracking-widest text-white print:text-black select-all py-1">
                    <?php echo e($license->key_code ?? 'ZBIZ-ORIGINAL-LICENSE'); ?>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6 bg-slate-900/50 p-6 rounded-2xl border border-slate-800/80 print:bg-slate-50 print:border-slate-300">
                <div class="space-y-3">
                    ...
                    <div class="space-y-1 text-xs">
                        <div class="text-white print:text-black font-bold text-sm"><?php echo e($tenant->name); ?></div>
                        <div class="text-slate-400 print:text-slate-600">NUIT: <span class="font-mono font-bold text-slate-200 print:text-black"><?php echo e($tenant->nuit ?? 'N/D'); ?></span></div>
                        <div class="text-slate-400 print:text-slate-600">Setor: <span class="capitalize text-slate-200 print:text-black"><?php echo e($tenant->business_type); ?></span></div>
                        <div class="text-slate-400 print:text-slate-600">Email: <span class="text-slate-200 print:text-black"><?php echo e($tenant->email ?? '-'); ?></span></div>
                    </div>
                </div>
                ...
                <div class="space-y-3">
                    ...
                    <div class="space-y-1 text-xs">
                        <div class="text-white print:text-black font-bold text-sm"><?php echo e($license->plan?->name ?? 'Plano Standard'); ?></div>
                        <div class="text-slate-400 print:text-slate-600">Modo de Operação: <span class="font-bold uppercase text-slate-200 print:text-black"><?php echo e($license->mode); ?></span></div>
                        <div class="text-slate-400 print:text-slate-600">Validade: <span class="font-mono font-bold text-emerald-400 print:text-emerald-700"><?php echo e($license->starts_at?->format('d/m/Y')); ?> a <?php echo e($license->expires_at?->format('d/m/Y')); ?></span></div>
                        <div class="text-slate-400 print:text-slate-600">Estado Atual: <span class="font-bold uppercase text-emerald-400 print:text-emerald-700"><?php echo e($license->status); ?></span></div>
                    </div>
                </div>
            </div>
            ...
            <div class="border-t border-slate-800 print:border-slate-300 pt-6 space-y-3 text-[11px] text-slate-400 print:text-slate-600">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                    <div>
                        ...
                        <span class="font-mono text-[10px] text-slate-300 print:text-black break-all"><?php echo e(substr($license->key_hash, 0, 48)); ?>...</span>
                    </div>
                    <div class="sm:text-right">
                        <span class="text-slate-500 font-bold uppercase text-[9px] block">Emitido Por</span>
                        <span class="text-slate-300 print:text-black font-bold"><?php echo e($license->issuer?->name ?? 'Administração Central ZBIZ+'); ?> (<?php echo e($license->created_at->format('d/m/Y H:i')); ?>)</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="border-t border-slate-800 print:border-slate-300 pt-8 mt-4 grid grid-cols-2 gap-8 text-center text-xs">
            ...
            <div class="space-y-1">
                ...
                <div class="text-[10px] text-slate-500"><?php echo e($tenant->name); ?></div>
            </div>
        </div>
    </div>
</div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/fdev-ms/Filipe/ZBIZ_PLUS/resources/views/owner/tenants/certificate.blade.php ENDPATH**/ ?>