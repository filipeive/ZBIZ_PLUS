<?php $__env->startSection('title', 'Nova Filial'); ?>
<?php $__env->startSection('page-title', 'Registar Nova Filial / Loja'); ?>
...
<div class="max-w-3xl mx-auto space-y-6">
    ...
    <div class="flex items-center justify-between bg-slate-900/80 border border-slate-800 rounded-3xl p-5 shadow-xl backdrop-blur-xl">
        <div class="flex items-center space-x-3">
            ...
            <div>
                <h2 class="text-lg font-black font-heading text-white">Adicionar Unidade</h2>
                <p class="text-xs text-slate-400">Registe uma nova loja, balcão de vendas ou armazém.</p>
            </div>
        </div>
    ...
    <div class="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl backdrop-blur-xl">
        <form method="POST" action="<?php echo e(route('branches.store')); ?>" class="space-y-6">
            <?php echo csrf_field(); ?>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="sm:col-span-2">
                    ...
                    <input type="text" name="name" value="<?php echo e(old('name')); ?>" required
                           placeholder="Ex: Filial Matola Rio, Armazém Central, Loja Shopping..."
...
unset($__errorArgs, $__bag); ?>
                ...
                <div>
                    ...
                    <input type="text" name="code" value="<?php echo e(old('code')); ?>" placeholder="Ex: MAT-01"
                ...
                <div>
                    ...
                    <input type="text" name="phone" value="<?php echo e(old('phone')); ?>" placeholder="Ex: +258 84 123 4567"
                           class="w-full px-3.5 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-sm text-white focus:ring-2 <?php echo e($theme['ring']); ?> outline-none">
                </div>

                <div>
                    <label class="block text-xs font-bold text-slate-300 mb-1">Email da Filial</label>
                    <input type="email" name="email" value="<?php echo e(old('email')); ?>" placeholder="Ex: matola@empresa.co.mz"
                           class="w-full px-3.5 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-sm text-white focus:ring-2 <?php echo e($theme['ring']); ?> outline-none">
                </div>

                <div>
                    <label class="block text-xs font-bold text-slate-300 mb-1">Endereço / Localização</label>
                    <input type="text" name="address" value="<?php echo e(old('address')); ?>" placeholder="Ex: Av. 25 de Setembro, Nº 123"
                           class="w-full px-3.5 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-sm text-white focus:ring-2 <?php echo e($theme['ring']); ?> outline-none">
                </div>

                <div class="sm:col-span-2 pt-2 space-y-3">
                    <label class="flex items-center gap-2 text-xs text-slate-300 cursor-pointer">
                        <input type="checkbox" name="is_main" value="1" <?php echo e(old('is_main') ? 'checked' : ''); ?> class="rounded bg-slate-950 border-slate-800 text-emerald-500">
                        <span>Definir como <strong>Matriz / Sede Principal</strong> da empresa</span>
                    </label>

                    <label class="flex items-center gap-2 text-xs text-slate-300 cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" <?php echo e(old('is_active', true) ? 'checked' : ''); ?> class="rounded bg-slate-950 border-slate-800 text-emerald-500">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/fdev-ms/Filipe/ZBIZ_PLUS/resources/views/branches/create.blade.php ENDPATH**/ ?>