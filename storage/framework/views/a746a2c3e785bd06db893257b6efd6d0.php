<?php $__env->startSection('page-title', 'Central de Notificações'); ?>
<?php $__env->startSection('title-icon', 'fa-bell'); ?>
...
<div class="space-y-6">
    ...
    <?php if($expiringBatches->count() > 0): ?>
        <div class="p-4 rounded-2xl bg-amber-500/10 dark:bg-amber-500/20 border border-amber-300/60 dark:border-amber-700/50 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-sm font-bold text-amber-900 dark:text-amber-200">
                        <?php echo e($expiringBatches->count()); ?> Lote(s) com Validade Próxima ou Expirados
                    </h3>
            ...
            <a href="<?php echo e(route('reports.low-stock')); ?>?tab=expiring" class="px-4 py-2 text-xs font-semibold rounded-xl bg-amber-600 hover:bg-amber-700 text-white transition-colors shrink-0 shadow-xs flex items-center gap-2">
        ...
        <div class="border-b border-slate-200 dark:border-slate-800 px-6 py-4 flex flex-wrap items-center justify-between gap-4">
            <div class="flex items-center gap-2">
                ...
                <button id="btn-tab-expiring" onclick="switchTab('expiring')" class="px-4 py-2 text-xs font-bold rounded-xl transition-all bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700">
                    <i class="fa-solid fa-calendar-xmark mr-1.5"></i> Alertas de Validade (<?php echo e($expiringBatches->count()); ?>)
                </button>
            </div>

            <?php if($unreadCount > 0): ?>
                <span class="px-3 py-1 text-xs font-semibold rounded-full bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300">
                    <?php echo e($unreadCount); ?> não lida(s)
                </span>
            <?php endif; ?>
        </div>
            ...
            <?php if($notifications->count() > 0): ?>
                <div class="space-y-3">
                    <?php $__currentLoopData = $notifications; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $n): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div id="notification-row-<?php echo e($n->id); ?>" class="p-4 rounded-xl border transition-all flex items-start justify-between gap-4 <?php echo e($n->read ? 'bg-slate-50/50 dark:bg-slate-900/40 border-slate-200/60 dark:border-slate-800/80 opacity-75' : 'bg-white dark:bg-slate-900 border-blue-200 dark:border-blue-900/50 shadow-xs'); ?>">
                            <div class="flex items-start gap-3">
                                <div class="w-10 h-10 rounded-xl flex items-center justify-center text-sm font-bold shrink-0 <?php echo e($n->read ? 'bg-slate-200/60 dark:bg-slate-800 text-slate-500' : 'bg-blue-500/10 text-blue-600 dark:bg-blue-500/20 dark:text-blue-400'); ?>">
                                    <i class="fa-solid <?php echo e($n->icon ?: 'fa-bell'); ?>"></i>
                                </div>
                                <div class="space-y-1">
                                    <div class="flex items-center gap-2">
                                        <h4 class="text-sm font-bold text-slate-800 dark:text-slate-100"><?php echo e($n->title); ?></h4>
                                        <?php if(!$n->read): ?>
                                            <span class="w-2 h-2 rounded-full bg-blue-600 inline-block"></span>
                                        <?php endif; ?>
                                    </div>
                                    <p class="text-xs text-slate-600 dark:text-slate-300"><?php echo e($n->message); ?></p>
                                    <span class="text-[11px] text-slate-400 dark:text-slate-500 block pt-1">
                                        <i class="fa-regular fa-clock mr-1"></i><?php echo e($n->created_at->diffForHumans()); ?> (<?php echo e($n->created_at->format('d/m/Y H:i')); ?>)
                                    </span>
                                </div>
                            </div>
                            ...
                            <div class="flex items-center gap-2 shrink-0">
                                <?php if($n->action_url): ?>
                                    <a href="<?php echo e($n->action_url); ?>" class="px-3 py-1.5 text-xs font-semibold rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 transition-colors">
                                        Ver Detalhes
                                    </a>
                                <?php endif; ?>

                                <?php if(!$n->read): ?>
                                    <button onclick="markAsRead(<?php echo e($n->id); ?>)" title="Marcar como lida" class="p-2 text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors">
                            ...
                            <?php $__currentLoopData = $expiringBatches; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $batch): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                ...
                                <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition-colors">
                                    <td class="py-3.5 px-4 font-bold text-slate-800 dark:text-slate-100">
                                        <div class="flex items-center gap-2">
                                            <div class="w-8 h-8 rounded-lg flex items-center justify-center text-xs font-bold <?php echo e($isExpired ? 'bg-rose-500/10 text-rose-600' : ($isCritical ? 'bg-amber-500/10 text-amber-600' : 'bg-blue-500/10 text-blue-600')); ?>">
                                                <i class="fa-solid fa-capsules"></i>
                                            </div>
                                            <div>
                                                <div><?php echo e($batch->product?->name ?? 'N/A'); ?></div>
                                                <div class="text-[11px] font-normal text-slate-400">Lote: <?php echo e($batch->batch_number ?: 'N/D'); ?></div>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="py-3.5 px-4 text-slate-600 dark:text-slate-400">
                                        <?php echo e($batch->product?->category?->name ?? 'Geral'); ?>

                                    </td>
                                    <td class="py-3.5 px-4 font-bold text-slate-700 dark:text-slate-300">
                                        <?php echo e(number_format($batch->quantity, 0)); ?> un
                                    </td>
                                    ...
                                    <td class="py-3.5 px-4">
                                        <?php if($isExpired): ?>
                                            <span class="px-2.5 py-1 text-[11px] font-bold rounded-lg bg-rose-100 text-rose-700 dark:bg-rose-900/40 dark:text-rose-300 inline-flex items-center gap-1">
                                                <i class="fa-solid fa-circle-xmark"></i> Expirado há <?php echo e(abs($daysRemaining)); ?> dia(s)
                                            </span>
                                        <?php elseif($isCritical): ?>
                                            <span class="px-2.5 py-1 text-[11px] font-bold rounded-lg bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300 inline-flex items-center gap-1">
                                                <i class="fa-solid fa-clock"></i> Expira em <?php echo e($daysRemaining); ?> dia(s)
                                            </span>
                                        <?php else: ?>
                                            <span class="px-2.5 py-1 text-[11px] font-semibold rounded-lg bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300 inline-flex items-center gap-1">
                                                <i class="fa-solid fa-calendar"></i> Expira em <?php echo e($daysRemaining); ?> dia(s)
                                            </span>
                                        <?php endif; ?>
                                    </td>
                                    <td class="py-3.5 px-4 text-right">
                                        <a href="<?php echo e(route('products.edit', $batch->product_id)); ?>" class="px-3 py-1.5 text-xs font-semibold rounded-lg bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 transition-colors">
                                            Gerenciar Produto
                                        </a>
                                    </td>
                                </tr>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/fdev-ms/Filipe/ZBIZ_PLUS/resources/views/notifications/index.blade.php ENDPATH**/ ?>