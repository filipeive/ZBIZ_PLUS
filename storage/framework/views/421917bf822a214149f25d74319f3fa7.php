<?php $__env->startSection('title', 'Folha Salarial Mensal'); ?>
<?php $__env->startSection('page-title', 'Folha Salarial & Vencimentos'); ?>
...
<div class="space-y-6" x-data="{
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-slate-900/80 border border-slate-800 rounded-3xl p-5 shadow-xl backdrop-blur-xl">
        <form method="GET" action="<?php echo e(route('users.employees.payroll')); ?>" class="flex items-center gap-3">
            <span class="text-xs font-bold text-slate-400 uppercase tracking-wider">Mês:</span>
            <input type="date" name="reference_month" id="reference_month_filter" value="<?php echo e($referenceMonth->format('Y-m-d')); ?>" class="px-3.5 py-2 bg-slate-950 border border-slate-800 rounded-xl text-white text-xs">
        ...
        <a href="<?php echo e(route('users.employees')); ?>" class="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold text-xs rounded-xl flex items-center gap-2 transition">
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4">
        <div class="p-5 rounded-3xl bg-slate-900/90 border border-slate-800 shadow-lg backdrop-blur-xl">
            ...
            <div class="text-2xl font-black font-mono text-white"><?php echo e($summary['employees']); ?></div>
        </div>
        <div class="p-5 rounded-3xl bg-slate-900/90 border border-slate-800 shadow-lg backdrop-blur-xl">
            <span class="text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1 block">Base Salarial</span>
            <div class="text-2xl font-black font-mono text-blue-400">MT <?php echo e(number_format($summary['base_total'], 2, ',', '.')); ?></div>
        </div>
        <div class="p-5 rounded-3xl bg-slate-900/90 border border-slate-800 shadow-lg backdrop-blur-xl">
            <span class="text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1 block">Total Pago no Mês</span>
            <div class="text-2xl font-black font-mono text-emerald-400">MT <?php echo e(number_format($summary['paid_total'], 2, ',', '.')); ?></div>
        </div>
        <div class="p-5 rounded-3xl bg-slate-900/90 border border-slate-800 shadow-lg backdrop-blur-xl">
            <span class="text-[10px] font-bold uppercase tracking-wider text-slate-500 mb-1 block">Pendente de Liquidação</span>
            <div class="text-2xl font-black font-mono <?php echo e($summary['balance_total'] > 0 ? 'text-rose-400' : 'text-emerald-400'); ?>">MT <?php echo e(number_format($summary['balance_total'], 2, ',', '.')); ?></div>
        </div>
    </div>

    <!-- Payroll Table -->
    <div class="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 shadow-xl backdrop-blur-xl overflow-hidden">
        <h3 class="text-sm font-bold text-white mb-4">Folha de Vencimentos: <?php echo e($referenceMonth->format('m/Y')); ?></h3>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-xs">
                ...
                <tbody class="divide-y divide-slate-800/60">
                    <?php $__empty_1 = true; $__currentLoopData = $payrollRows; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $row): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-slate-800/30 transition">
                            <td class="py-3.5">
                                <div class="font-bold text-white"><?php echo e($row['employee']->employee_label); ?></div>
                                <div class="text-[10px] text-slate-500 font-mono"><?php echo e($row['employee']->email); ?></div>
                            </td>
                            <td class="py-3.5 text-slate-300"><?php echo e($row['employee']->job_title ?: '-'); ?></td>
                            <td class="py-3.5 text-right font-mono font-bold text-white">MT <?php echo e(number_format($row['base_salary'], 2, ',', '.')); ?></td>
                            <td class="py-3.5 text-right font-mono font-bold text-emerald-400">MT <?php echo e(number_format($row['paid_amount'], 2, ',', '.')); ?></td>
                            <td class="py-3.5 text-right font-mono font-bold <?php echo e($row['balance'] > 0 ? 'text-rose-400' : 'text-slate-400'); ?>">
                                MT <?php echo e(number_format($row['balance'], 2, ',', '.')); ?>
                            ...
                            <td class="py-3.5 text-center">
                                ...
                                ?>
                                <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase border <?php echo e($statusBadge); ?>">
                            ...
                            <td class="py-3.5 text-right">
                                <div class="inline-flex items-center gap-1.5">
                                    <?php if(userCan('manage_finances')): ?>
                                        <button type="button" @click="empId = <?php echo e($row['employee']->id); ?>; empName = '<?php echo e(addslashes($row['employee']->name)); ?>'; baseSalary = <?php echo e($row['base_salary']); ?>; balance = <?php echo e($row['balance']); ?>; showPayModal = true" class="px-3 py-1 bg-emerald-500/20 text-emerald-400 hover:bg-emerald-500/30 border border-emerald-500/30 rounded-xl text-xs font-bold transition">
                                            Pagar
                                        </button>
                                    <?php endif; ?>
                                    <a href="<?php echo e(route('users.show', $row['employee'])); ?>" class="w-7 h-7 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 flex items-center justify-center text-xs">
            ...
            <form :action="`/users/${empId}/salary-payments`" method="POST" class="space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    <div>
                        ...
                        <input type="date" name="reference_month" value="<?php echo e($referenceMonth->format('Y-m-d')); ?>" class="w-full px-3.5 py-2 bg-slate-950 border border-slate-800 rounded-xl text-white text-xs">
                    </div>
                ...
                <div class="flex justify-end gap-2 pt-2 border-t border-slate-800">
                    ...
                    <button type="submit" class="px-5 py-2 rounded-xl bg-gradient-to-r <?php echo e($theme['gradient']); ?> text-slate-950 font-black text-xs">Confirmar Registo</button>
                </div>
            </form>
        </div>
    </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /home/fdev-ms/Filipe/ZBIZ_PLUS/resources/views/users/payroll.blade.php ENDPATH**/ ?>