        ...
        <aside :class="sidebarOpen ? 'translate-x-0 shadow-2xl' : '-translate-x-full lg:translate-x-0'"
            ...
            <div class="flex flex-col flex-1 min-h-0 overflow-y-auto">
                <div class="p-5 border-b border-slate-800/80 flex items-center justify-between">
                    <a href="{{ $isOwnerConsole ? route('owner.tenants.index') : route('dashboard.index') }}" class="flex items-center space-x-3 group">
                        ...
                        <div>
                            ...
                            <div class="text-[10px] uppercase font-bold tracking-wider text-slate-400 truncate max-w-[130px]" title="{{ $isOwnerConsole ? 'Owner Console' : ($tenant?->name ?? 'ZBIZ ERP') }}">
                    ...
                    <!-- Sector Pill Badge -->
                    @if($isOwnerConsole)
                        <span class="text-[10px] uppercase font-bold tracking-widest px-2 py-0.5 rounded-full border {{ $theme['badge'] }} flex items-center gap-1" title="Gestão SaaS">
                ...
                <div class="px-4 py-3 border-b border-slate-800/80">
                    <a href="{{ route('pos.index') }}" 
                       class="w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl bg-gradient-to-r {{ $theme['gradient'] }} hover:opacity-95 text-white font-bold transition shadow-lg shadow-emerald-500/10 border border-white/20">
                    ...
                    <div class="flex items-center space-x-3 min-w-0">
                        ...
                        <div class="min-w-0">
                            ...
                            <div class="text-[10px] text-slate-500 truncate flex items-center gap-1">
                                ...
                                <span>{{ $isOwnerConsole ? 'Owner Console' : ($branch?->name ?? 'Loja Principal') }}</span>
                            </div>
                        </div>
                    </div>
            ...
            <header class="h-16 flex-shrink-0 bg-slate-900/80 border-b border-slate-800/80 backdrop-blur-md px-4 sm:px-8 flex items-center justify-between z-20 shadow-sm">
                ...
                <div class="flex items-center space-x-2.5 sm:space-x-3">
                    ...
                    <div class="hidden sm:flex items-center space-x-1.5 bg-slate-950/80 border border-slate-800/80 px-3 py-1.5 rounded-xl text-xs" title="Console do dono do sistema">
                        <i class="fa-solid fa-building-shield {{ $theme['text_accent'] }}"></i>
                        <span class="text-slate-300 font-semibold">Owner Console</span>
                    </div>
                    @elseif(auth()->user()->canSwitchBranch())
