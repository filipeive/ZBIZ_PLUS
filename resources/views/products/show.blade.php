        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-800">
            <div>
                ...
                <div class="text-xs text-slate-400 mt-1 flex flex-wrap items-center gap-3">
                    <span>Código de Barras: <strong class="text-white font-mono">{{ $product->barcode ?? 'N/D' }}</strong></span>
                    <span>SKU: <strong class="text-white font-mono">{{ $product->sku ?? ('PRD-' . $product->id) }}</strong></span>
                    <span>Tipo: <strong class="text-slate-300">{{ $product->type === 'service' ? 'Serviço' : 'Produto Físico' }}</strong></span>
                </div>
            </div>
                        ...
                        <tbody class="divide-y divide-slate-800/60 bg-slate-900/60">
                            @forelse($product->batches()->orderBy('expiry_date', 'asc')->get() as $b)
                                ...
                                <tr>
                                    ...
                                    <td class="p-3 font-mono text-slate-300">{{ $b->expiry_date ? $b->expiry_date->format('d/m/Y') : 'N/D' }}</td>
                        ...
                        <tbody class="divide-y divide-slate-800/60 bg-slate-900/60">
                            @foreach($product->stockMovements()->latest()->take(5)->get() as $mov)
                                <tr>
                                    ...
                                    <td class="p-3 text-right font-mono font-bold {{ $mov->movement_type === 'out' ? 'text-rose-400' : 'text-emerald-400' }}">
                                        {{ $mov->movement_type === 'out' ? '-' : '+' }}{{ abs($mov->quantity) }}
                                    </td>
                                    <td class="p-3 text-slate-300">{{ $mov->reason ?? 'Movimentação regular' }}</td>
                                    <td class="p-3 text-slate-400">{{ $mov->user?->name ?? 'Sistema' }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
