...
@section('title', 'Mesas do Restaurante')
@section('page-title', 'Mapa de Mesas')
...
<div class="space-y-6" x-data="{ showModal: false }">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        @forelse($tables as $table)
            ...
            <div class="bg-slate-900/90 border rounded-3xl p-5 shadow-xl backdrop-blur-xl flex flex-col justify-between transition-all duration-200 group relative overflow-hidden {{ $statusConfig['glow'] }}">
                ...
                <div>
                    ...
                    <div class="flex items-center justify-between mb-3">
                        <div class="flex items-center gap-2.5">
                            ...
                            <div>
                                <h3 class="text-lg font-black text-white font-heading">{{ $table->name }}</h3>
                                <p class="text-[11px] text-slate-400 flex items-center gap-1">
                                    <i class="fa-solid fa-users text-slate-500"></i> {{ $table->capacity }} lugares
                                </p>
                            </div>
                        </div>

                        <span class="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider border {{ $statusConfig['badge'] }}">
                            {{ $statusConfig['label'] }}
                        </span>
                    </div>
                        ...
                        <div class="mt-4 p-3.5 bg-slate-950/80 border border-rose-500/20 rounded-2xl space-y-2">
                            <div class="flex items-center justify-between text-xs">
                                <span class="font-bold text-amber-400 flex items-center gap-1">
                                    <i class="fa-solid fa-receipt"></i> Pedido #{{ $activeOrder->id }}
                                </span>
                                <span class="px-2 py-0.5 rounded-full text-[10px] font-bold border {{ $activeOrder->status_badge }}">
                                    {{ $activeOrder->status_text }}
                                </span>
                            </div>

                            <div class="flex items-center justify-between text-xs text-slate-300">
                                <span>Itens registados:</span>
                                <strong class="text-white">{{ $activeOrder->items->count() }} itens</strong>
                            </div>
                            ...
                            <div class="flex items-center justify-between text-xs text-slate-300 pt-1 border-t border-slate-800">
                                ...
                                <strong class="text-emerald-400 font-mono font-bold text-sm">{{ number_format($activeOrder->estimated_amount, 2, ',', '.') }} MT</strong>
                            </div>
                        </div>
                    @else
                ...
                <div class="mt-5 pt-4 border-t border-slate-800/80 space-y-3">
                    ...
                    <div class="flex items-center gap-2">
                        @if($hasActiveOrder)
                            <a href="{{ route('orders.edit', $activeOrder->id) }}" class="flex-1 py-2.5 rounded-xl bg-sky-500 hover:bg-sky-600 text-slate-950 font-black text-xs text-center transition shadow-md shadow-sky-500/10 flex items-center justify-center gap-1.5">
                                <i class="fa-solid fa-pen-to-square"></i> Ver / Editar Pedido
                            </a>

                            <form method="POST" action="{{ route('restaurant.tables.clear', $table) }}" onsubmit="return confirm('Deseja libertar esta mesa e concluir a conta?');">
                                @csrf
                        ...
                        @else
                            <form method="POST" action="{{ route('restaurant.tables.create-order', $table) }}" class="flex-1">
                                @csrf
                        ...
                        @if(!$hasActiveOrder)
                            <form method="POST" action="{{ route('restaurant.tables.destroy', $table) }}" onsubmit="return confirm('Tem certeza que deseja apagar esta mesa?');">
                                @csrf
                                @method('DELETE')
                    ...
                    <form method="POST" action="{{ route('restaurant.tables.status', $table) }}">
                        ...
                        <select name="status" onchange="this.form.submit()" class="w-full bg-slate-950 border border-slate-800 rounded-xl px-3 py-1.5 text-xs text-slate-300 focus:outline-none cursor-pointer">
                            <option value="free" @selected($table->status === 'free')>Livre</option>
                            <option value="occupied" @selected($table->status === 'occupied')>Ocupada</option>
                            <option value="reserved" @selected($table->status === 'reserved')>Reservada</option>
                            <option value="cleaning" @selected($table->status === 'cleaning')>Em Limpeza</option>
                        </select>
                    </form>
    ...
    <div x-cloak x-show="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
        <div @click.away="showModal = false" class="bg-slate-900 border border-slate-800 rounded-3xl p-6 max-w-md w-full shadow-2xl space-y-4">
            ...
            <form method="POST" action="{{ route('restaurant.tables.store') }}" class="space-y-4">
                @csrf
                <div>
                ...
                <div class="flex gap-3 pt-2">
                    ...
                    <button type="submit" class="w-2/3 py-2.5 rounded-xl {{ $theme['btn'] }} text-xs font-bold transition">Guardar Mesa</button>
                </div>
            </form>
        </div>
    </div>
