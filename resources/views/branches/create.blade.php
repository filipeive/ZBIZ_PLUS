...
@section('page-title', 'Registar Nova Filial / Loja')
        ...
        <form method="POST" action="{{ route('branches.store') }}" class="space-y-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="text" name="code" value="{{ old('code') }}" placeholder="Ex: MAT-01"
                ...
                <div>
                    ...
                    <input type="email" name="email" value="{{ old('email') }}" placeholder="Ex: matola@empresa.co.mz"
                           class="w-full px-3.5 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-sm text-white focus:ring-2 {{ $theme['ring'] }} outline-none">
                </div>

                <div>
                    <label class="block text-xs font-bold text-slate-300 mb-1">Endereço / Localização</label>
                    <input type="text" name="address" value="{{ old('address') }}" placeholder="Ex: Av. 25 de Setembro, Nº 123"
                           class="w-full px-3.5 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-sm text-white focus:ring-2 {{ $theme['ring'] }} outline-none">
                </div>

                <div class="sm:col-span-2 pt-2 space-y-3">
                    <label class="flex items-center gap-2 text-xs text-slate-300 cursor-pointer">
                        <input type="checkbox" name="is_main" value="1" {{ old('is_main') ? 'checked' : '' }} class="rounded bg-slate-950 border-slate-800 text-emerald-500">
                        <span>Definir como <strong>Matriz / Sede Principal</strong> da empresa</span>
                    </label>

                    <label class="flex items-center gap-2 text-xs text-slate-300 cursor-pointer">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="rounded bg-slate-950 border-slate-800 text-emerald-500">
