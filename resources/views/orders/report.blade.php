...
@section('title', 'Relatório de Pedidos & Encomendas')
@section('page-title', 'Relatório de Pedidos & Encomendas')
...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-slate-900/80 border border-slate-800 rounded-3xl p-5 sm:p-6 shadow-xl backdrop-blur-xl">
        ...
        <div class="flex items-center gap-2.5 w-full sm:w-auto">
            <a href="{{ route('orders.index') }}" class="px-4 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold text-xs rounded-xl flex items-center gap-2 transition">
        ...
        <div class="bg-slate-900/90 border border-slate-800 rounded-3xl p-5 shadow-lg backdrop-blur-xl">
            ...
            <div class="mt-4">
                <div class="text-2xl sm:text-3xl font-black font-heading text-white">
                    {{ $reportStats['total_orders'] ?? 0 }} <span class="text-xs text-slate-400 font-normal">pedidos</span>
                </div>
                <div class="text-xs text-slate-500 mt-1">
                    {{ $reportStats['by_status']['completed'] ?? 0 }} concluídos
                </div>
            </div>
        </div>
        ...
        <div class="bg-slate-900/90 border border-slate-800 rounded-3xl p-5 shadow-lg backdrop-blur-xl">
            ...
            <div class="mt-4">
                <div class="text-2xl sm:text-3xl font-black font-heading text-emerald-400 font-mono">
                    {{ number_format($reportStats['total_amount'] ?? 0, 2, ',', '.') }} <span class="text-xs text-slate-400 font-normal">MT</span>
                </div>
                <div class="text-xs text-slate-500 mt-1">
                    {{ number_format($reportStats['total_advance'] ?? 0, 2, ',', '.') }} MT adiantados
                </div>
            </div>
        </div>
        ...
        <div class="bg-slate-900/90 border border-slate-800 rounded-3xl p-5 shadow-lg backdrop-blur-xl">
            ...
            <div class="mt-4">
                <div class="text-2xl sm:text-3xl font-black font-heading text-amber-400 font-mono">
                    {{ number_format($reportStats['total_pending'] ?? 0, 2, ',', '.') }} <span class="text-xs text-slate-400 font-normal">MT</span>
                </div>
                <div class="text-xs text-slate-500 mt-1">
        ...
        <div class="bg-slate-900/90 border border-slate-800 rounded-3xl p-5 shadow-lg backdrop-blur-xl">
            ...
            <div class="mt-4">
                <div class="text-2xl sm:text-3xl font-black font-heading text-rose-400">
                    {{ $reportStats['overdue_count'] ?? 0 }} <span class="text-xs text-slate-400 font-normal">pedidos</span>
                </div>
    ...
    <div class="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 shadow-xl backdrop-blur-xl">
        <form method="GET" action="{{ route('orders.report') }}" class="space-y-4">
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                ...
                <div>
                    ...
                    <select name="priority" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white focus:ring-2 {{ $theme['ring'] }} outline-none">
                ...
                <div>
                    ...
                    <input type="text" name="customer" value="{{ request('customer') }}" placeholder="Nome do cliente..."
                ...
                <div class="flex items-end gap-2">
                    <button type="submit" class="w-full px-4 py-2 {{ $theme['btn'] }} text-xs rounded-xl transition flex items-center justify-center gap-1.5">
                        <i class="fa-solid fa-filter"></i> Filtrar
                    </button>
                    @if(request()->hasAny(['status', 'priority', 'customer', 'date_from', 'date_to']))
                        <a href="{{ route('orders.report') }}" class="px-3 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl text-xs flex items-center justify-center" title="Limpar">
    ...
    <div class="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 shadow-xl backdrop-blur-xl overflow-hidden">
        <div class="flex items-center justify-between pb-4 border-b border-slate-800 mb-4">
            <div>
                <h3 class="text-base font-black font-heading text-white">Listagem de Encomendas ({{ $orders->count() }})</h3>
                    ...
                    @forelse($orders as $ord)
                        ...
                        <tr class="hover:bg-slate-800/30 transition">
                            <td class="py-3 font-mono text-slate-400">
                                #{{ str_pad($ord->id, 5, '0', STR_PAD_LEFT) }}
                            </td>
                            <td class="py-3 font-bold text-white">
                                {{ $ord->customer_name ?? $ord->customer?->name ?? 'Cliente' }}
                            </td>
                            <td class="py-3 text-slate-400 font-mono">
                                {{ $ord->created_at ? $ord->created_at->format('d/m/Y') : '-' }}
                            </td>
                            <td class="py-3 font-mono text-slate-300">
                                {{ $ord->delivery_date ? \Carbon\Carbon::parse($ord->delivery_date)->format('d/m/Y') : 'Imediato' }}
                            </td>
                            <td class="py-3 text-center">
                                <span class="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase {{ $ord->status === 'completed' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/30' : ($ord->status === 'in_progress' ? 'bg-sky-500/10 text-sky-400 border border-sky-500/30' : 'bg-amber-500/10 text-amber-400 border border-amber-500/30') }}">
                                    {{ $ord->status === 'completed' ? 'Concluído' : ($ord->status === 'in_progress' ? 'Produção' : 'Pendente') }}
                                </span>
                            </td>
                            <td class="py-3 text-right font-mono font-bold text-white">
                                {{ number_format($ord->estimated_amount, 2, ',', '.') }} MT
                            </td>
                            <td class="py-3 text-right font-mono text-emerald-400">
                                {{ number_format($ord->advance_payment, 2, ',', '.') }} MT
                            </td>
                            <td class="py-3 text-right font-mono font-bold {{ $pending > 0 ? 'text-amber-400' : 'text-slate-500' }}">
                                {{ number_format($pending, 2, ',', '.') }} MT
                            </td>
                            <td class="py-3 text-right">
                                <a href="{{ route('orders.show', $ord->id) }}" class="inline-flex items-center justify-center w-7 h-7 rounded-lg bg-slate-800 text-slate-300 hover:text-white hover:bg-slate-700 transition" title="Ver Detalhes">
