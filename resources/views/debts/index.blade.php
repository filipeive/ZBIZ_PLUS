...
@section('content')
<div class="space-y-6" x-data="{ viewMode: window.innerWidth < 1024 ? 'grid' : (localStorage.getItem('preferredView_debts') || 'table') }">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 bg-slate-900/80 border border-slate-800 rounded-3xl p-5 shadow-xl backdrop-blur-xl">
        ...
        <div class="flex items-center gap-2.5 w-full sm:w-auto justify-end">
            ...
            <div class="bg-slate-950 border border-slate-800 rounded-2xl p-1 flex items-center gap-1">
                <button @click="viewMode = 'grid'; localStorage.setItem('preferredView_debts', 'grid')" :class="viewMode === 'grid' ? 'bg-slate-800 text-white shadow-sm' : 'text-slate-400 hover:text-white'" class="px-3 py-1.5 rounded-xl text-xs font-bold transition flex items-center gap-1.5" title="Modo Cartão (Mobile)">
                    <i class="fa-solid fa-border-all"></i> Grid
                </button>
                <button @click="viewMode = 'table'; localStorage.setItem('preferredView_debts', 'table')" :class="viewMode === 'table' ? 'bg-slate-800 text-white shadow-sm' : 'text-slate-400 hover:text-white'" class="px-3 py-1.5 rounded-xl text-xs font-bold transition flex items-center gap-1.5" title="Modo Tabela (Desktop)">
            ...
            <a href="{{ route('debts.create') }}" class="px-5 py-2.5 rounded-2xl bg-amber-500 hover:bg-amber-600 text-slate-950 font-bold text-xs shadow-sm transition flex items-center gap-2">
            ...
            <div class="bg-slate-900/90 border border-slate-800 rounded-3xl p-5 shadow-lg backdrop-blur-xl flex flex-col justify-between hover:border-slate-700 transition group relative overflow-hidden">
                <div>
                    <div class="flex items-center justify-between mb-3">
                        ...
                        <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold border {{ $debt->remaining_amount <= 0 ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/30' : 'bg-amber-500/10 text-amber-400 border-amber-500/30' }}">
                    ...
                    <p class="text-xs text-slate-400 mt-1 font-mono">
                        <i class="fa-solid fa-phone text-slate-500 mr-1"></i> {{ $debt->customer_phone ?? $debt->customer?->phone ?? '-' }}
                    </p>

                    <div class="mt-3 pt-3 border-t border-slate-800/80 flex items-center justify-between text-xs text-slate-400">
                        <span>Original: <strong class="text-slate-300 font-mono">{{ number_format($debt->original_amount ?? $debt->total_amount, 2, ',', '.') }} MT</strong></span>
                        <span>Pago: <strong class="text-emerald-400 font-mono">{{ number_format($debt->paid_amount ?? 0, 2, ',', '.') }} MT</strong></span>
                    </div>
                </div>
                ...
                <div class="mt-5 pt-4 border-t border-slate-800/80 flex items-center justify-between">
                    <div>
                        ...
                        <span class="text-base font-black text-rose-400 font-mono">{{ number_format($debt->remaining_amount, 2, ',', '.') }} MT</span>
                    </div>

                    <div class="flex items-center gap-2">
                        <a href="{{ route('debts.show', $debt->id) }}" class="w-8 h-8 rounded-xl bg-slate-800 text-slate-400 hover:text-white flex items-center justify-center transition" title="Ver Extrato">
