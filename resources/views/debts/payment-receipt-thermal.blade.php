...
<head>
    ...
    <title>Recibo de Pagamento Térmico #{{ $payment->id }}</title>
    <style>
        @page {
            margin: 0;
            size: 80mm auto;
...
<body>
    <div class="no-print">
        ...
        <a href="{{ route('debts.payments.receipt', ['payment' => $payment->id, 'format' => 'a4']) }}" class="btn-print" style="background: #059669; text-align: center; display: block; text-decoration: none; margin-bottom: 6px;">📄 VER VERSÃO A4 OFICIAL</a>
    ...
    <div><strong>Ref. Dívida:</strong> #DIV-{{ str_pad($debt->id, 6, '0', STR_PAD_LEFT) }}</div>
    ...
    <table class="table">
        ...
        <tr>
            ...
            <td class="text-right" style="font-weight: bold;">{{ strtoupper($payment->payment_method_text ?? $payment->payment_method ?? 'Numerário') }}</td>
        </tr>
        @if($payment->notes)
        <tr>
