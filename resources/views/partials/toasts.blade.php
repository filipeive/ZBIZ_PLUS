        ...
        init() {
            @if(session('toast_message'))
                this.addToast({
                    ...
                    message: {!! json_encode(session('toast_message')) !!}
                });
            @endif

            @if(session('success'))
                this.addToast({ type: 'success', title: 'Sucesso', message: {!! json_encode(session('success')) !!} });
            @endif

            @if(session('error'))
                this.addToast({ type: 'error', title: 'Erro', message: {!! json_encode(session('error')) !!} });
            @endif

            @if(session('warning'))
                this.addToast({ type: 'warning', title: 'Atenção', message: {!! json_encode(session('warning')) !!} });
            @endif
