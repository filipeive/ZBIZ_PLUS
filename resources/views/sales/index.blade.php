...
@section('content')
<div class="space-y-6" x-data="{ viewMode: window.innerWidth < 1024 ? 'grid' : (localStorage.getItem('preferredView_sales') || 'table') }">
    ...
    <div class="bg-slate-900/90 border border-slate-800 rounded-3xl p-5 shadow-xl backdrop-blur-xl space-y-4">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-2">
                    ...
                    @if(current_branch())
                        <span class="px-2.5 py-0.5 rounded-full text-[11px] font-bold bg-slate-800 border border-slate-700 text-slate-300">
                            <i class="fa-solid fa-store text-emerald-400 mr-1"></i> {{ current_branch()->name }}
                        </span>
                    @endif
                </div>
            ...
            <div class="flex flex-wrap items-center gap-2.5">
                ...
                <div class="bg-slate-950 border border-slate-800 rounded-2xl p-1 flex items-center gap-1">
                    <button @click="viewMode = 'grid'; localStorage.setItem('preferredView_sales', 'grid')" :class="viewMode === 'grid' ? 'bg-slate-800 text-white shadow-sm' : 'text-slate-400 hover:text-white'" class="px-3 py-1.5 rounded-xl text-xs font-bold transition flex items-center gap-1.5" title="Modo Cartão (Mobile / Tablet)">
                        <i class="fa-solid fa-border-all"></i> Grid
                    </button>
                    <button @click="viewMode = 'table'; localStorage.setItem('preferredView_sales', 'table')" :class="viewMode === 'table' ? 'bg-slate-800 text-white shadow-sm' : 'text-slate-400 hover:text-white'" class="px-3 py-1.5 rounded-xl text-xs font-bold transition flex items-center gap-1.5" title="Modo Tabela (Desktop)">
                        <i class="fa-solid fa-list"></i> Tabela
                    </button>
                </div>

                <a href="{{ route('reports.sales-specialized') }}" class="px-4 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white font-bold text-xs border border-slate-700 transition flex items-center gap-2">
                ...
                @endif
                <a href="{{ route('pos.index') }}" class="px-5 py-2.5 rounded-2xl bg-emerald-500 hover:bg-emerald-600 text-white font-bold text-xs shadow-lg shadow-emerald-500/20 transition flex items-center gap-2">
        ...
        <!-- Filter Controls -->
        <form method="GET" action="{{ route('sales.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 pt-3 border-t border-slate-800/80 items-end">
            <div class="lg:col-span-2 relative">
                ...
                <div class="relative">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Nome do cliente ou telefone..."
                           class="w-full pl-9 pr-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white placeholder-slate-600 outline-none focus:ring-2 {{ $theme['ring'] }}">
                </div>
            </div>
            ...
            <div>
                ...
                <input type="date" name="date_from" value="{{ request('date_from') }}"
                       class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white outline-none focus:ring-2 {{ $theme['ring'] }}">
            </div>
            ...
            <div class="flex items-center gap-2">
                ...
                <a href="{{ route('sales.index') }}" class="py-2 px-3 bg-slate-800 text-slate-400 hover:text-white rounded-xl text-xs font-bold transition">
                    Limpar
                </a>
                @endif
            </div>
            ...
            <div class="bg-slate-900/90 border border-slate-800 rounded-3xl p-5 shadow-lg backdrop-blur-xl flex flex-col justify-between hover:border-slate-700 transition group relative overflow-hidden">
                <div>
                    ...
                    <p class="text-xs text-slate-400 mt-1 flex items-center gap-1">
                        <i class="fa-solid fa-user text-slate-500"></i> {{ $sale->user?->name ?? 'Operador' }}
                    </p>

                    <div class="mt-3 pt-3 border-t border-slate-800/80 flex items-center justify-between text-xs text-slate-400 font-mono">
                        <span><i class="fa-regular fa-calendar mr-1"></i>{{ $sale->created_at ? $sale->created_at->format('d/m/Y') : '-' }}</span>
                        <span><i class="fa-regular fa-clock mr-1"></i>{{ $sale->created_at ? $sale->created_at->format('H:i') : '00:00' }}</span>
                    </div>
                </div>
                ...
                <div class="mt-5 pt-4 border-t border-slate-800/80 flex items-center justify-between">
                    <div>
                        ...
                        <span class="text-base font-black text-white font-mono">{{ number_format($sale->total_amount, 2, ',', '.') }} MT</span>
                    </div>

                    <div class="flex items-center gap-2">
                        <a href="{{ route('sales.show', $sale->id) }}" class="w-8 h-8 rounded-xl bg-slate-800 text-slate-300 hover:text-white flex items-center justify-center transition" title="Ver Detalhes">
                            <i class="fa-solid fa-eye text-xs"></i>
                        </a>
                        <a href="{{ route('pos.receipt', $sale->id) }}" target="_blank" class="w-8 h-8 rounded-xl bg-slate-800 text-slate-400 hover:text-white flex items-center justify-center transition" title="Imprimir Recibo">
                        ...
                        <tr class="hover:bg-slate-800/30 transition">
                            ...
                            <td class="py-3.5 text-slate-300 font-mono text-xs">
                                ...
                                <div class="text-[10px] text-slate-400">
                                    <i class="fa-regular fa-clock mr-1 text-emerald-400"></i>{{ $sale->created_at ? $sale->created_at->format('H:i:s') : ($sale->sale_date ? \Carbon\Carbon::parse($sale->sale_date)->format('H:i') : '00:00') }}
                                </div>
                            </td>
                            ...
                            <td class="py-3.5 text-right">
                                <div class="flex items-center justify-end gap-1.5">
                                    <a href="{{ route('sales.show', $sale->id) }}" class="w-8 h-8 rounded-xl bg-slate-800 text-slate-300 hover:text-white hover:bg-slate-700 flex items-center justify-center transition" title="Ver Detalhes">
                                        <i class="fa-solid fa-eye text-xs"></i>
                                    </a>
                                    <a href="{{ route('pos.receipt', $sale->id) }}" target="_blank" class="w-8 h-8 rounded-xl bg-slate-800 text-slate-400 hover:text-white hover:bg-slate-700 flex items-center justify-center transition" title="Imprimir Recibo">
                                        <i class="fa-solid fa-receipt text-xs"></i>
                                    </a>
                                </div>
                            </td>
