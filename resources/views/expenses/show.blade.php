...
@section('page-title', 'Extrato da Despesa #' . $expense->id)
...
<div class="max-w-4xl mx-auto space-y-6" x-data="{ showUploadModal: false }">
    ...
    <div class="flex items-center justify-between bg-slate-900/80 border border-slate-800 rounded-3xl p-5 shadow-xl backdrop-blur-xl">
        ...
        <div class="flex items-center gap-2">
            <a href="{{ route('expenses.edit', $expense) }}" class="px-4 py-2 bg-slate-800 hover:bg-slate-700 text-amber-400 font-bold text-xs rounded-xl flex items-center gap-2 border border-slate-700 transition">
    ...
    <div class="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl backdrop-blur-xl space-y-6">
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-slate-800">
            <div>
                ...
                <h2 class="text-2xl font-black font-heading text-white">{{ $expense->description }}</h2>
                <div class="text-xs text-slate-400 mt-1">Categoria: <span class="text-slate-200 font-bold">{{ $expense->category?->name ?? 'Geral' }}</span></div>
            </div>
            ...
            <div class="sm:text-right">
                ...
                <div class="text-3xl font-black font-heading font-mono text-rose-400">
                    {{ number_format($expense->amount, 2, ',', '.') }} <span class="text-xs text-slate-400">MT</span>
                </div>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 p-4 rounded-2xl bg-slate-950 border border-slate-800 text-xs">
            <div>
                ...
                <div class="font-bold text-white mt-0.5">{{ $expense->expense_date->format('d/m/Y') }}</div>
            </div>
            <div>
                <div class="text-slate-500 font-bold uppercase text-[10px]">Conta Financeira</div>
                <div class="font-bold text-slate-200 mt-0.5">{{ $expense->financialAccount?->name ?? 'Caixa Principal' }}</div>
            </div>
            <div>
                <div class="text-slate-500 font-bold uppercase text-[10px]">Registado Por</div>
                <div class="font-bold text-slate-200 mt-0.5">{{ $expense->user?->name ?? 'Sistema' }}</div>
            </div>
        </div>
        ...
        @if($expense->notes)
            <div class="p-4 rounded-2xl bg-slate-950/60 border border-slate-800 text-xs">
                ...
                <p class="text-slate-300 leading-relaxed">{{ $expense->notes }}</p>
            </div>
        @endif
            ...
            @if($expense->hasReceiptFile())
                <div class="p-4 rounded-2xl bg-slate-950 border border-slate-800 flex items-center justify-between">
                    <div class="flex items-center space-x-3">
                        ...
                        <div>
                            <div class="text-xs font-bold text-white">{{ basename($expense->receipt_file_path) }}</div>
                            <div class="text-[10px] text-slate-500">Documento anexado</div>
                        </div>
                    </div>
                    <a href="{{ Storage::url($expense->receipt_file_path) }}" target="_blank" class="px-3 py-1.5 bg-slate-800 hover:bg-slate-700 text-white rounded-xl text-xs font-bold transition flex items-center gap-1">
    ...
    <div x-cloak x-show="showUploadModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
        <div @click.away="showUploadModal = false" class="bg-slate-900 border border-slate-800 rounded-3xl p-6 max-w-md w-full shadow-2xl space-y-4">
            ...
            <form action="{{ route('expenses.receipt.upload', $expense->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                @csrf
                <div>
