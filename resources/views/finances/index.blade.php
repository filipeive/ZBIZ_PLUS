...
@section('page-title', 'Gestão Financeira, Contas & Livro-Razão')
...
<div class="space-y-6" x-data="{ showModal: false, filterOpen: false }">
    ...
    <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-4 bg-slate-900/90 border border-slate-800 rounded-3xl p-5 shadow-xl backdrop-blur-xl">
        ...
        <div class="flex flex-wrap items-center gap-2">
            ...
            <a href="{{ route('expenses.index') }}" class="px-3.5 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-bold transition flex items-center gap-1.5">
                <i class="fa-solid fa-money-bill-wave text-rose-400"></i> Despesas
            </a>
            <a href="{{ route('debts.index') }}" class="px-3.5 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-bold transition flex items-center gap-1.5">
                <i class="fa-solid fa-hand-holding-dollar text-amber-400"></i> Dívidas
            </a>
            @if(App\Helpers\PermissionHelper::userCan('view_reports'))
                <a href="{{ route('reports.inventory') }}" class="px-3.5 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-bold transition flex items-center gap-1.5">
                    <i class="fa-solid fa-chart-pie text-sky-400"></i> Relatórios
                </a>
            @endif
            @if(App\Helpers\PermissionHelper::userCan('manage_finances'))
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div class="bg-slate-900/90 border border-slate-800 rounded-3xl p-5 shadow-xl backdrop-blur-xl relative overflow-hidden group">
            ...
            <div class="text-xl font-black font-heading text-blue-400">
                {{ number_format($totalRealValue ?? ($currentCapital + $receivables), 2, ',', '.') }} <span class="text-xs text-slate-400 font-normal">MT</span>
            </div>
        ...
        <div class="bg-slate-900/90 border border-slate-800 rounded-3xl p-5 shadow-xl backdrop-blur-xl relative overflow-hidden group">
            ...
            <div class="flex items-center justify-between mb-2">
                <span class="text-[10px] font-extrabold uppercase tracking-wider text-slate-400">Liquidez Atual</span>
            ...
            <span class="text-[10px] text-slate-500 block mt-1">Soma atual de caixa, carteiras e contas operacionais</span>
        </div>
        ...
        <div class="bg-slate-900/90 border border-slate-800 rounded-3xl p-5 shadow-xl backdrop-blur-xl relative overflow-hidden group">
            ...
            <div class="text-xl font-black font-heading text-amber-400">
                {{ number_format($receivables ?? 0, 2, ',', '.') }} <span class="text-xs text-slate-400 font-normal">MT</span>
            </div>
        ...
        <div class="bg-slate-900/90 border border-slate-800 rounded-3xl p-5 shadow-xl backdrop-blur-xl relative overflow-hidden group">
            ...
            <div class="text-xl font-black font-heading text-emerald-400">
                {{ number_format($monthSummary['inflows'] ?? 0, 2, ',', '.') }} <span class="text-xs text-slate-400 font-normal">MT</span>
            </div>
        ...
        <div class="bg-slate-900/90 border border-slate-800 rounded-3xl p-5 shadow-xl backdrop-blur-xl relative overflow-hidden group">
            ...
            <div class="text-xl font-black font-heading text-rose-400">
                {{ number_format($monthSummary['outflows'] ?? 0, 2, ',', '.') }} <span class="text-xs text-slate-400 font-normal">MT</span>
            </div>
    ...
    <div class="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 shadow-xl backdrop-blur-xl space-y-4">
        <div class="flex items-center justify-between border-b border-slate-800 pb-3">
            ...
            <span class="text-xs text-slate-400 font-mono">{{ count($accounts ?? []) }} Contas Ativas</span>
        </div>
            ...
            @forelse($accounts ?? [] as $account)
                <div class="bg-slate-950 border border-slate-800 rounded-2xl p-4 flex flex-col justify-between hover:border-slate-700 transition">
                        ...
                        <div class="flex items-center justify-between mb-2">
                            <div class="flex items-center gap-2.5">
                                <div class="w-8 h-8 rounded-xl bg-slate-900 text-slate-300 flex items-center justify-center text-xs font-bold border border-slate-800">
                                    @if($account->type === 'bank') <i class="fa-solid fa-building-columns text-blue-400"></i>
                                    @elseif($account->type === 'mobile_money') <i class="fa-solid fa-mobile-screen-button text-amber-400"></i>
                                    @else <i class="fa-solid fa-wallet text-emerald-400"></i> @endif
                                </div>
                                <div>
                                    <h4 class="text-xs font-bold text-white leading-tight">{{ $account->name }}</h4>
                                    <span class="text-[9px] uppercase tracking-wider text-slate-500 font-mono">{{ str_replace('_', ' ', $account->type) }}</span>
                                </div>
                            </div>
                            <span class="px-2 py-0.5 rounded-full text-[9px] font-bold uppercase {{ $account->current_balance >= 0 ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : 'bg-rose-500/10 text-rose-400 border border-rose-500/20' }}">
                    ...
                    <div class="mt-3 pt-2 border-t border-slate-800/80 flex items-center justify-between text-[10px] text-slate-500">
                        <span>Inicial: {{ number_format($account->opening_balance ?? 0, 2, ',', '.') }} MT</span>
                        <span>{{ $account->branch?->name ?? 'Principal' }}</span>
                    </div>
                </div>
            @empty
    ...
    <div class="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 shadow-xl backdrop-blur-xl overflow-hidden space-y-4">
        ...
        <div x-show="filterOpen" x-transition class="p-4 bg-slate-950 rounded-2xl border border-slate-800">
            <form method="GET" action="{{ route('finances.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                <div>
                    <label class="block text-[10px] font-bold uppercase text-slate-400 mb-1">Período de Datas</label>
                    <div class="grid grid-cols-2 gap-1.5">
                        <input type="date" name="date_from" value="{{ $filters['date_from'] ?? '' }}" class="px-2.5 py-1.5 bg-slate-900 border border-slate-800 rounded-xl text-xs text-white">
                        <input type="date" name="date_to" value="{{ $filters['date_to'] ?? '' }}" class="px-2.5 py-1.5 bg-slate-900 border border-slate-800 rounded-xl text-xs text-white">
                    </div>
                </div>
                    ...
                    <div class="grid grid-cols-2 gap-1.5">
                        <select name="financial_account_id" class="px-2.5 py-1.5 bg-slate-900 border border-slate-800 rounded-xl text-xs text-white">
                            ...
                            @foreach($accounts ?? [] as $acc)
                                <option value="{{ $acc->id }}" @selected((string)($filters['financial_account_id'] ?? '') === (string)$acc->id)>{{ $acc->name }}</option>
                            @endforeach
                        </select>
                ...
                <div>
                    ...
                    <input type="text" name="search" value="{{ $filters['search'] ?? '' }}" placeholder="Procurar transação..." class="w-full px-3 py-1.5 bg-slate-900 border border-slate-800 rounded-xl text-xs text-white">
                </div>
                ...
                <div class="flex items-end gap-2">
                    ...
                    <a href="{{ route('finances.index') }}" class="px-3 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-bold rounded-xl transition">Limpar</a>
                </div>
            </form>
        </div>
                    ...
                    @forelse($transactions ?? [] as $tx)
                        ...
                        <tr class="hover:bg-slate-800/30 transition">
                            <td class="py-3.5 font-mono text-slate-400">
                                {{ $tx->transaction_date ? \Carbon\Carbon::parse($tx->transaction_date)->format('d/m/Y') : ($tx->created_at ? $tx->created_at->format('d/m/Y H:i') : '-') }}
                            </td>
    ...
    <div x-cloak x-show="showModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
        <div @click.away="showModal = false" class="bg-slate-900 border border-slate-800 rounded-3xl p-6 max-w-lg w-full shadow-2xl space-y-4">
            ...
            <form action="{{ route('finances.transactions.store') }}" method="POST" class="space-y-4">
                @csrf
                <div>
                    <label class="block text-xs font-bold text-slate-300 mb-1">Conta Financeira *</label>
                    <select name="financial_account_id" required class="w-full px-3.5 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white outline-none focus:ring-2 {{ $theme['ring'] }}">
                        @foreach($accounts ?? [] as $acc)
                            <option value="{{ $acc->id }}">{{ $acc->name }} (Saldo: {{ number_format($acc->current_balance, 2, ',', '.') }} MT)</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        ...
                        <select name="type" required class="w-full px-3.5 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white outline-none focus:ring-2 {{ $theme['ring'] }}">
                            @foreach($manualTransactionTypes ?? [] as $key => $tInfo)
                                <option value="{{ $key }}">{{ $tInfo['label'] }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" step="0.01" name="amount" min="0.01" required placeholder="0.00"
                               class="w-full px-3.5 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white outline-none focus:ring-2 {{ $theme['ring'] }}">
                    </div>
                </div>
                ...
                <div>
                    ...
                    <textarea name="notes" rows="2" placeholder="Notas adicionais de auditoria..."
                              class="w-full px-3.5 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white outline-none focus:ring-2 {{ $theme['ring'] }}"></textarea>
                </div>
                ...
                <div class="flex gap-3 pt-2">
                    ...
                    <button type="submit" class="w-2/3 py-2.5 rounded-xl {{ $theme['btn'] }} text-xs font-bold hover:scale-105 active:scale-95 transition">Registar Lançamento</button>
                </div>
            </form>
        </div>
    </div>
