...
@section('content')
<div class="space-y-6" x-data="{ showModal: false, viewMode: window.innerWidth < 1024 ? 'grid' : (localStorage.getItem('preferredView_stock_movements') || 'table') }">
    ...
    <div class="bg-slate-900/90 border border-slate-800 rounded-3xl p-5 shadow-xl backdrop-blur-xl space-y-4">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <div class="flex flex-wrap items-center gap-2.5 w-full sm:w-auto justify-end">
                ...
                <div class="bg-slate-950 border border-slate-800 rounded-2xl p-1 flex items-center gap-1">
                    <button @click="viewMode = 'grid'; localStorage.setItem('preferredView_stock_movements', 'grid')" :class="viewMode === 'grid' ? 'bg-slate-800 text-white shadow-sm' : 'text-slate-400 hover:text-white'" class="px-3 py-1.5 rounded-xl text-xs font-bold transition flex items-center gap-1.5" title="Modo Cartão (Mobile)">
                        <i class="fa-solid fa-border-all"></i> Grid
                    </button>
                    <button @click="viewMode = 'table'; localStorage.setItem('preferredView_stock_movements', 'table')" :class="viewMode === 'table' ? 'bg-slate-800 text-white shadow-sm' : 'text-slate-400 hover:text-white'" class="px-3 py-1.5 rounded-xl text-xs font-bold transition flex items-center gap-1.5" title="Modo Tabela (Desktop)">
                        <i class="fa-solid fa-list"></i> Tabela
                    </button>
                </div>

                <a href="{{ route('reports.inventory') }}" class="px-4 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white font-bold text-xs border border-slate-700 transition flex items-center gap-2">
        ...
        <!-- Filter Controls -->
        <form method="GET" action="{{ route('stock-movements.index') }}" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 pt-3 border-t border-slate-800/80 items-end">
            <div class="relative">
            ...
            <div>
                ...
                <select name="movement_type" onchange="this.form.submit()" class="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white outline-none focus:ring-2 {{ $theme['ring'] }}">
            ...
            <div class="flex items-center gap-2">
                ...
                <a href="{{ route('stock-movements.index') }}" class="py-2 px-3 bg-slate-800 text-slate-400 hover:text-white rounded-xl text-xs font-bold transition">
                    Limpar
                </a>
                @endif
            </div>
            ...
            <div class="bg-slate-900/90 border border-slate-800 rounded-3xl p-5 shadow-lg backdrop-blur-xl flex flex-col justify-between hover:border-slate-700 transition group relative overflow-hidden">
                <div>
                    <div class="flex items-center justify-between mb-3">
                        <span class="px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase border {{ $m->movement_type === 'in' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/30' : ($m->movement_type === 'out' ? 'bg-rose-500/10 text-rose-400 border-rose-500/30' : 'bg-amber-500/10 text-amber-400 border-amber-500/30') }}">
                        ...
                        <span class="text-xs font-mono font-black {{ $m->movement_type === 'in' ? 'text-emerald-400' : ($m->movement_type === 'out' ? 'text-rose-400' : 'text-amber-400') }}">
                ...
                <div class="mt-5 pt-4 border-t border-slate-800/80 flex items-center justify-between text-xs text-slate-400">
                    <span class="font-mono"><i class="fa-regular fa-clock text-slate-500 mr-1"></i> {{ $m->created_at ? $m->created_at->format('d/m/Y H:i') : '-' }}</span>
                    <span class="font-bold text-slate-300"><i class="fa-solid fa-user text-slate-500 mr-1"></i> {{ $m->user?->name ?? 'Sistema' }}</span>
                </div>
            </div>
        @empty
